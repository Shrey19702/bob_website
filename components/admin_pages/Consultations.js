...
const Consultations = () => {
  ...
  const getConsultations = async () => {
    const data = await fetch(`${process.env.BASE_URL}api/consultation/getConsultation`, {
      method: 'GET'
    })
    const info = await data.json();
    setConsultation(info.body);
  }

  const deleteConsultation = async (id) => {
    const data = await fetch(`${process.env.BASE_URL}api/consultation/deleteConsultation`, {
      method: "DELETE",
      body: JSON.stringify({
        _id: id
      })
  ...
  useEffect(() => {
    if (!consultation) { getConsultations(); }
  })

  //    if(consultation.length() == 0)
  //    return(<>
  //     No info currently present
  //    </>)

  return (
    <div className=" flex flex-col">
      {
        consultation && consultation.map((x, idx)=> (
          <div key={idx} className="flex justify-center">
            <div className="rounded-lg shadow-lg bg-white max-w-sm">
              <div className="p-6">
                ...
                <h5 className="text-gray-900 text-xl font-medium mb-2">{x._id}</h5>
