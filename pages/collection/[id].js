...
export default function Category({f_collection}) {
  f_collection = JSON.parse(f_collection);
  ...
  return (
    <div className="bg-white">
      <div className=' pt-20'>
        <h2 className="text-3xl max-w-2xl mx-auto px-4 space-x-2 sm:px-6 lg:max-w-7xl lg:px-8 font-semibold text-gray-900">{f_collection.name}</h2>
        <div className="pt-4">
          <nav aria-label="Breadcrumb">
            <ol role="list" className="max-w-2xl mx-auto px-4 flex items-center space-x-2 sm:px-6 lg:max-w-7xl lg:px-8">
              {createBreadcrumbs(f_collection).map(breadcrumb => {
                if (breadcrumb.id == 2) {
                  return (
      ...
      {f_collection.categories.map(name=>{
          return (
            <Products name={name} />
          )
      })}
